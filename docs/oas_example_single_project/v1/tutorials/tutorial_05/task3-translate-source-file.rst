...
..  list-table::
    ...
    * - **Start Translation Job**
      - `POST /job`_
      - Initiates an asynchronous process to translate a specified model to another format.
    * - **Fetch Manifest**
      - `GET /{urn}/manifest`_
      - Retrieves the manifest for the model specified by the ``urn`` URI parameter.
...
When you start a translation job, you specify the Base64-encoded URN of the source file. You must also specify the translated file format you require, which is SVF2 in this case. You can optionally specify what geographic region the translated file must be stored.
...
There are two ways to check the status of a translation job. The first is to set up a webhook that notifies you when the job is done. The second is to poll the status of the job periodically. For this tutorial, you will be polling the status of the translation job. This means that you must periodically inspect the manifest produced by the translation job. The ``status`` attribute in the manifest reports the status of the translation job. The status can be:
...
.. code-block:: json
  :emphasize-lines: 33-34, 43, 52-53, 62, 71, 80, 81, 97, 101, 118, 165,169
